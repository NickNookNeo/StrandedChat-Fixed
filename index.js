...
io.on('connection', function(socket) {
  ...
  socket.on('adduser', function(user, admin, vcid, fn2) {
    ...
    if (admin == 'banned') {
      ...
      room = 'ban';
      socket.join('ban');
      socket.emit('serverChat', 'You are in Jail.');
    ...
    socket.broadcast.emit('serverChat', username + ' has joined us!');
  ...
  socket.on('chat message', function(msg, img) {
    ...
    if (msg.includes("<script")) {
      socket.emit('serverChat', 'Don\'t hack!');
      return;
    }
    ...
    io.in(room).emit('chat message', msg.replaceAll("<script", ""), 'black', img);
    console.log(msg);
  });
  ...
  socket.on('vc', function(user) {
    // when user requests voicechat, send voice chat data from requested user
    user = user.replace('_', ' ');
    if (user in usernames) {
